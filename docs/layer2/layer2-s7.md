...
**Step 1 — Resource Reference validation.**
...
**Step 2 — Capability Declaration currency validation.**
...
**Step 3 — Pre-Arrangement Declaration validation.**
...
**Step 4 — Delegation Topology feasibility step (multi-party bookings only).**
...
**Step 6 — Emit result.**
On `FEASIBILITY_CLEARED` from the supplier (and with all steps 1–4 passed): emit `FEASIBILITY_CLEARED` for this Activity Component, including any warnings accumulated in steps 1–3. On any failure in steps 1–4 or a supplier `FEASIBILITY_FAILED` in step 5: emit `FEASIBILITY_FAILED` with the accumulated reason set.
